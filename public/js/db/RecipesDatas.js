...
export default class modelData {
		...
		DATAS.forEach(data => {
			if (!!data.ingredients && data.ingredients.length > 0) {
				data.ingredients.forEach(ingredient => {
					if (inResult.indexOf(ingredient.ingredient.toLowerCase()) === -1) {
						result.push({
							type: "ingredient",
							name: ingredient.ingredient.toLowerCase(),
						});
						inResult.push(ingredient.ingredient.toLowerCase());
					}
				});
			}
		});
	...
	static findAllAppliances() {
		let result = [];
		let inResult = [];
		DATAS.forEach(data => {
			if (!!data.appliance) {
				if (inResult.indexOf(data.appliance.toLowerCase()) === -1) {
					result.push({
						type: "appliance",
						name: data.appliance.toLowerCase(),
					});
					inResult.push(data.appliance.toLowerCase());
				}
			}
		});
		return result;
	...
	static findAllUstensils() {
		let result = [];
		let inResult = [];
		DATAS.forEach(data => {
			if (!!data.ustensils && data.ustensils.length > 0) {
				data.ustensils.forEach(ustensil => {
					if (inResult.indexOf(ustensil.toLowerCase()) === -1) {
						result.push({
							type: "ustensil",
							name: ustensil.toLowerCase(),
						});
						inResult.push(ustensil.toLowerCase());
					}
				});
			}
		});
